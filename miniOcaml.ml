...
let tytostr ty =
  let rec tytostr ty = match ty with
    | Int -> "int"
    | Bool -> "bool"
    | Pair (ty1, ty2) -> (tytostr ty1) ^ " * " ^ (tytostr ty2)
    | Arrow (ty1, ty2) -> (tytostr ty1) ^ " -> " ^ (tytostr ty2)
  in tytostr ty
...
let tlisttostr l =            
  let rec tlisttostr l =
    match l with
      | tk::[] -> fsttokentostr [tk]
      | tk::l -> (fsttokentostr [tk]) ^ "; " ^ (tlisttostr l)
      | [] -> ""

  in "[" ^ (tlisttostr l) ^ "]"

let inttostr n =
  let digittostr d = if (d >= 0) && (d < 10) then String.make 1 (Char.chr ((Char.code '0') + d)) else failwith "[DigittoStr] Parameter is not a digit(0-9)"
  in let rec inttostr n = if n < 1 then "" else (inttostr (n / 10)) ^ (digittostr (n mod 10))
  in inttostr n

let valtostr v =
  let booltostr b = match b with
    | true -> "true"
    | false -> "false"

  in let rec valtostr v = match v with
    | Bval(b) -> booltostr b
    | Ival(n) -> inttostr n
    | Closure(v, e, env) -> "(" ^ v ^ ", <exp>, <env>)"
    | Rclosure(var1, var2, e, env) -> "(" ^ var1 ^ "," ^ var2 ^ ", <exp>, <env>)"
    | Pair(v1, v2) -> "(" ^ (valtostr v1) ^ ", " ^ (valtostr v2) ^ ")"
  in valtostr v
let optostr o = match o with
  | Add -> "+"
  | Sub -> "-"
  | Mul -> "*"
  | Leq -> "<="
...
let chartodigit c = (Char.code c) - (Char.code '0')
let lex s : token list =
  ...
  let rec lex i l =
    if exhausted i then List.rev l
      ...
      | c when digit c -> lex_num (i+1) (chartodigit c) l
  ...
  and lex_num i n l = if exhausted i then lex_num' i n l
      ...
      |c when digit c -> lex_num (i+1) ((n*10) + (chartodigit c)) l
      |_ -> lex_num' i n l
...
let parsetype l : ty * (token list) =
  ...
  and pty l = match l with
    ...
    | l -> failwith ("[Parsetype] Unexpected Token: " ^ (fsttokentostr l))
  ...
  let rec exp l = match l with
    ...
    | LAM::LP::VAR(x)::COL::l -> begin
      ...
      match l with
        | RP::ARR::l -> begin
          let (e, l) = exp l in
          (Lamty(x, ty, e), l) end
        | l -> failwith ("[Parse] Lam: Unexpected Token: Expected ')' and '->', but got " ^ (fsttokentostr l)) end
    | LET::VAR(x)::EQ::l -> begin
      ...
      match l with
        | IN::l -> begin
          let (e2, l) = exp l in
          (Let(x, e1, e2), l) end
        | l -> failwith ("[Parse] Let: Unexpected Token: Expected 'in', but got " ^ (fsttokentostr l)) end
    | LET::LP::VAR(x1)::COM::VAR(x2)::RP::EQ::l -> begin
      ...
      match l with
        ...
        | l -> failwith ("[Parse] Letpair: Unexpected Token: Expected 'in', but got " ^ (fsttokentostr l)) end
    | LET::REC::VAR(f)::VAR(x)::EQ::l -> begin
      ...
      match l with
        ...
        | l -> failwith ("[Parse] Letrec: Unexpected Token: Expected 'in', but got " ^ (fsttokentostr l)) end
            ...
            match l with
              | EQ::l -> begin
                ...
                match l with
                  ...
                  | l -> failwith ("[Parse] Letrecty: Unexpected Token: Expected 'in', but got " ^ (fsttokentostr l)) end
              | l -> failwith ("[Parse] Letrecty: Unexpected Token: Expected '=', but got " ^ (fsttokentostr l)) end
        | l -> failwith ("[Parse] Letrecty: Unexpected Token: Expected ')' and ':', but got " ^ (fsttokentostr l)) end
    | l -> cexp l
      ...
      |THEN::l -> begin
        ...
        match l with
          |ELSE::l -> begin
            let (e3, l) = exp l in
            (If(e1, e2, e3), l) end
          | l -> failwith ("[Parse] If: Unexpected Token: Expected 'else', but got " ^ (fsttokentostr l)) end
      | _ -> failwith ("[Parse] IF: Unexpected Token: Expected 'then', but got " ^ (fsttokentostr l))
    ...
    | LP::l -> begin
      ...
      in begin match l with
        ...
        | COM::l -> begin
          ...
          match l with
            | RP::l -> (Pairexp(e, e2), l)
            | l -> failwith ("[Parse] Missing parenthesis: Expected ')', but got " ^ (fsttokentostr l)) end
        | l -> failwith ("[Parse] Missing parenthesis: Expected ')' or ',', but got " ^ (fsttokentostr l)) end end
    | l -> failwith ("[Parse] Unexpected Token: " ^ (tlisttostr l))

  in exp l

let check env e : ty =
  let check_op (o: op) t1 t2 : ty = match o with
    | Add -> if (t1 = Int) && (t2 = Int) then Int else failwith ("[Check] Oapp: Wrong type(s): Expected 'int' and 'int', got " ^ (tytostr t1) ^ " and " ^ (tytostr t2))
    | Sub -> if (t1 = Int) && (t2 = Int) then Int else failwith ("[Check] Oapp: Wrong type(s): Expected 'int' and 'int', got " ^ (tytostr t1) ^ " and " ^ (tytostr t2))
    | Mul -> if (t1 = Int) && (t2 = Int) then Int else failwith ("[Check] Oapp: Wrong type(s): Expected 'int' and 'int', got " ^ (tytostr t1) ^ " and " ^ (tytostr t2))
    | Leq -> if (t1 = Int) && (t2 = Int) then Bool else failwith ("[Check] Oapp: Wrong type(s): Expected 'int' and 'int', got " ^ (tytostr t1) ^ " and " ^ (tytostr t2)) in
  
  let check_fun t1 t2 = match t1 with
    | Arrow(ty1, ty2) -> if t2 = ty1 then ty2 else failwith ("[Check] Fapp: Argument type and given type don't match: Expected " ^ (tytostr ty1) ^ ", got " ^ (tytostr t2))
    | ty -> failwith ("[Check] Fapp: Expected: Expected a function, got expression of type " ^ (tytostr ty)) in
  ...
  let rec check env e : ty = match e with
    ...
    | Bool -> begin let t1 = check env e2 in
        ...
        if t1 = t2 then t1
        else failwith ("[Check] If: then and else expressions are not of the same type: Got " ^ (tytostr t1) ^ " and " ^ (tytostr t2)) end
      | ty -> failwith ("[Check] If: Condition is not of type 'bool': Got " ^ (tytostr ty)) end
    ...
    | Letpair(x1, x2, e1, e2) -> begin let ty = check env e1 in
      match ty with
        ...
        | ty -> failwith ("[Check] Letpair: Expected expression of type 'Pair', got expression of type " ^ (tytostr ty)) end
    ...
    | Letrecty(f, x, t1, t2, e1, e2) -> let t2' = (check (update (update env x t1) f (Arrow(t1, t2))) e1) in
      ...
      else failwith ("[Check] Return type of Function (" ^ (tytostr t2') ^ ") does not match expected return type (" ^ (tytostr t2) ^ ")")
  in check env e
...
let eval (env : (var, value) env) e =
  let rec eval env e = match e with
    ...
    | If(e1, e2, e3) -> begin match eval env e1 with
      ...
      | v -> failwith ("[Eval] If: Expected 'Bool' value, but got: " ^ (valtostr v)) end
    ...
    | Letpair(x1, x2, e1, e2) -> begin let v = eval env e1 in
      match v with
        ...
        | v -> failwith ("[Eval] Letpair: Expected 'Pair' value, but got: " ^ (valtostr v)) end
  ...
  and eval_op op v1 v2 = match op, v1, v2 with
    ...
    | o, _, _ -> failwith ("[Eval] Oapp: Illegal values for operation " ^ (optostr o))
  ...
  and eval_fun v1 v2 = match v1 with
    ...
    | v -> failwith ("[Eval] Fapp: Expected function, got: " ^ (valtostr v))
